...
if (!code.includes('Avatar')) {
    code = code.replace('    Typography,\n} from "@mui/material";', '    Typography,\n    Avatar,\n    Chip,\n    Divider,\n} from "@mui/material";');
}

if (!code.includes('CameraIcon')) {
    code = code.replace('    WrenchIcon,\n} from "@common/icons";', '    WrenchIcon,\n    CameraIcon,\n} from "@common/icons";');
}

const bookingCardMatch = code.match(/function BookingCard\(props: BookingCardProps\) \{[\s\S]*?\}\n\}/);
if (bookingCardMatch) {
    let newBookingCard = `function BookingCard(props: BookingCardProps) {
    ...
    code = code.replace(bookingCardMatch[0], newBookingCard);
}
