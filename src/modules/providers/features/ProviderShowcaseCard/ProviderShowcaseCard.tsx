...
function getSubtitle(provider: Provider): string {
    const isMaster = isMasterProvider(provider);
    if (isMaster) {
        return (
    ...
    return (
        provider.professions?.[0] ||
        getCategoryLabel(getProviderType(provider))
    );
}
        ...
        <>
            <MasterHead>
                ...
                <Identity>
                    ...
                    <ProviderSubtitle>{getSubtitle(provider)}</ProviderSubtitle>
                    <MetaRow>
