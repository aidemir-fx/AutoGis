...
type Props = {
    ...
    isLoading: boolean;
    height?: string;
    onSelectMaster?: (provider: Provider) => void;
};
...
function getDetailsHref(provider: Provider): string {
    return `javascript:void((window as any).handleProviderClick('${provider.id}', '${getProviderType(provider)}'))`;
}
            ...
            <BalloonBody>
                ...
                {shouldShowBookingAction(provider) ? (
                    <BalloonActions $layout="booking">
                        <BalloonAction href="#" onClick={(e) => { e.preventDefault(); if((window as any).handleProviderClick) (window as any).handleProviderClick(provider.id, getProviderType(provider)); }} $variant="booking">
                            <CalendarIcon />
                            Записаться
                        </BalloonAction>
                        <BalloonAction
                    ...
                    <BalloonActions>
                        ...
                        <BalloonAction href="#" onClick={(e) => { e.preventDefault(); if((window as any).handleProviderClick) (window as any).handleProviderClick(provider.id, getProviderType(provider)); }} $variant="outline">
                            <EyeIcon />
                            Подробнее
                        </BalloonAction>
                    </BalloonActions>
    ...
    return (
        <div style={{ position: "relative", width: "100%", height: height || "100%" }}>
            <YMaps query={{ apikey: import.meta.env.VITE_YMAPS_API_KEY }}>
                {!isReady && <MapStub />}
                <MapContainer $isReady={isReady} style={{ height: height || "100%" }}>
                    <Map
                        state={mapState}
                        width="100%"
                        height={height || "100%"}
